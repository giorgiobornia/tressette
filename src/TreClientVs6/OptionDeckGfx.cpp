...
*/
void OptionDeckGfx::Init(SDL_Rect* pRect, SDL_Surface*  pScreen, TTF_Font* pFontTxt, TTF_Font* pFontWinCtrl)
{
    ASSERT(pRect);
    ...
    m_pComboDeck->Init(&rctBt1, pScreen,  m_pFontText, MYIDCOMBODECK);
...
*/
void  OptionDeckGfx::loadCardPac(cTipoDiMazzo::eTypeMazzo eType, 
                                  SDL_Surface** ppsurf_DestDeck,  cCardGfx* pCard)
{
    Uint32          timetag;
...
void OptionDeckGfx::Show(SDL_Surface* pScene_background)
{
    ...
    while (!m_bTerminated)
    {
        ...
        GFX_UTIL::DrawString(pShadowSrf, strDeckSelectTitle.c_str(), m_pComboDeck->m_rctButt.x , 
                    m_pComboDeck->m_rctButt.y - 20, GFX_UTIL_COLOR::Orange , m_pFontText);
