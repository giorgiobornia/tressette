...
void Space<xsl:value-of select="name"/>::GetProfile(const char* strIniFileName)
{
  ...
  <xsl:for-each select="//section">
    ...
    <xsl:for-each select="item_string">
    g_Options.<xsl:value-of select="$sectobj"/>.<xsl:value-of select="name"/> = GetPrivateProfileString(lpsz<xsl:value-of select="$namesect"/>, lpsz<xsl:value-of select="$namesect"/><xsl:value-of select="name"/>, "<xsl:value-of select="default"/>",buff, 2048,strIniFileName);
    </xsl:for-each>
  </xsl:for-each>
}
</xsl:template> 
