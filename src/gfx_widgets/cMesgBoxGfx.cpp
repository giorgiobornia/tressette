...
*/
void cMesgBoxGfx::Init(SDL_Rect* pRect, SDL_Surface*  pScreen, TTF_Font* pFont, eMSGBOX_TYPE eval )
{
    ASSERT(pRect);
    ...
    SDL_SetAlpha(m_pSurf_Bar, SDL_SRCALPHA, 70);
    ...
    if (m_eTypeMsg == MB_YES_NO)
    {
        ...
        m_pBtButt1->Init(&rctBt1, pScreen,  pFont, MB_ID_BUTT_YES);
        ...
        m_pBtButt2->Init(&rctBt1, pScreen,  pFont, MB_ID_BUTT_NO);
    ...
    else if (m_eTypeMsg == MBOK)
    {
        ...
        m_pBtButt1->Init(&rctBt1, pScreen,  pFont, 0);
        m_pBtButt1->SetState(cButtonGfx::INVISIBLE);
        m_pBtButt1->m_fncbClickEvent = MakeDelegate(this, &cMesgBoxGfx::ButCmdClicked);
    }
    else
